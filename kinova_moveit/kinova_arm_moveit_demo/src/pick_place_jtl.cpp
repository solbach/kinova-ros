...
PickPlaceJTL::PickPlaceJTL(ros::NodeHandle &nh):
    nh_(nh)
{
...
//    }
    ...
    if (robot_connected_)
    {
        //sub_joint_ = nh_.subscribe<sensor_msgs::JointState>("/j2s7s300_driver/out/joint_state", 1, &PickPlaceJTL::get_current_state, this);
...
void PickPlaceJTL::get_current_state(const sensor_msgs::JointStateConstPtr &msg)
{
    boost::mutex::scoped_lock lock(mutex_state_);
    current_state_ = *msg;
}

void PickPlaceJTL::get_current_pose(const geometry_msgs::PoseStampedConstPtr &msg)
{
...
void PickPlaceJTL::clear_workscene()
{
    // remove table
    co_.id = "table";
...
void PickPlaceJTL::build_workscene()
{
...
void PickPlaceJTL::clear_obstacle()
{
    co_.id = "pole";
    co_.operation = moveit_msgs::CollisionObject::REMOVE;
    pub_co_.publish(co_);
...
void PickPlaceJTL::add_obstacle()
{
    clear_obstacle();
...
void PickPlaceJTL::add_complex_obstacle()
{
    clear_obstacle();
...
void PickPlaceJTL::add_attached_obstacle()
{
...
void PickPlaceJTL::check_collision()
{
...
void PickPlaceJTL::define_cartesian_pose()
{
    tf::Quaternion q;
...
void PickPlaceJTL::define_joint_values()
{
 ...
 */
geometry_msgs::PoseStamped PickPlaceJTL::generate_gripper_align_pose(geometry_msgs::PoseStamped targetpose_msg, double dist, double azimuth, double polar, double rot_gripper_z)
{
...
void PickPlaceJTL::setup_constrain(geometry_msgs::Pose target_pose, bool orientation, bool position)
{
...
void PickPlaceJTL::check_constrain()
{
...
void PickPlaceJTL::evaluate_plan(moveit::planning_interface::MoveGroup &group)
{
    bool replan = true;
    int count = 0;
...
void PickPlaceJTL::getInvK(geometry_msgs::Pose &eef_pose, std::vector<double> &joint_value)
{
...
{
    ros::init(argc, argv, "pick_place_jtl");
    ros::NodeHandle node;
    ros::AsyncSpinner spinner(1);
    spinner.start();

    kinova::PickPlaceJTL pick_place_jtl(node);
